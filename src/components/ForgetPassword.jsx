...
const ForgotPassword = () => {
  ...
  const handleVerify = async () => {
    if (!employeeId) {
      ...
      setErrors({});
      try {
        const response = await axios.post(`https://hrms-application-l1e0.onrender.com/hrmsapplication/authentication/forgot-password`, {
  ...
  const handleChangePassword = async () => {
    if (createPassword !== confirmPassword || createPassword.length < 8) {
      ...
      try {
        await axios.patch("https://hrms-application-l1e0.onrender.com/hrmsapplication/authentication/resetPassword", {
          employeeId: employeeId,
